...
is registered (`import "@lilydesignsystem/web-components-locale-picker"`) and
...
```ts
import { LocalePicker } from "@lilydesignsystem/web-components-locale-picker";
...
```ts
import { isRtlLocale } from "@lilydesignsystem/web-components-locale-picker";
router.afterEach(() => {
