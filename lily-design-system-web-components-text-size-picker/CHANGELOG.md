# Changelog — `<lily-text-size-picker>` (Web Components helper)
...
relative to `lily-design-system-web-components-text-size-picker`'s last published version (`0.1.0`);
...
(`@lilydesignsystem/html-text-size-picker`), differing only in the tag it
registers (`<lily-text-size-picker>` rather than `<text-size-picker>`) and its package name.
...
`@lilydesignsystem/web-components-text-size-picker`. The version resets to
